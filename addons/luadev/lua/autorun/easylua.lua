...
/**
	EasyLua Sandbox Extensions
	Created by Potatofactory <mrpotatofactory@gmail.com>

	For development use on Synergyst Community 
**/
...
if SERVER then
	...
	util.AddNetworkString ("sound.PlayURL")
	util.AddNetworkString ("sound.PlayFile")
	util.AddNetworkString ("player/Console")
	util.AddNetworkString ("player/Notify")
	util.AddNetworkString ("player/NotifyProgress")
	util.AddNetworkString ("player/NotifyProgressKill")

	chat    = chat    and chat    or {}
	sound   = sound   and sound   or {}
	sandbox = sandbox and sandbox or {}


	sandbox.Notify = function (strText, iType, iLength, pTarget)
		if not iType then
			iType = NOTIFY_GENERIC
		end
		if not iLength then
			iLength = 8
		end

		net.Start ("player/Notify")
			net.WriteString (strText)
			net.WriteInt    (iType, 3)
			net.WriteInt    (iLength, 6)
		if pTarget then
			net.Send (pTarget)
		else
			net.Broadcast ()
	...
	sandbox.NotifyProgress = function (strText, iLength, pTarget)
		strID = uuid.new ()
		...
		net.Start ("player/NotifyProgress")
			net.WriteString (strText)
			net.WriteString (strID)
			net.WriteInt    (iLength, 32)
		if pTarget then
			net.Send (pTarget)
		else
			net.Broadcast ()
	...
	sandbox.KillProgress = function (strID, pTarget)
		net.Start ("player/NotifyProgressKill")
			net.WriteString (strID)
		if pTarget then
			net.Send (pTarget)
		else
	...
	function pMeta:Console (strText)
		net.Start ("player/Console")
			net.WriteString (strText)
		net.Send  (self)
	end

	function pMeta:Notify (strText, iType, iLength)
		sandbox.Notify (strText, iType, iLength, self)
	end

	function pMeta:NotifyProgress (strText, iLength)
		return sandbox.NotifyProgress (strText, iLength, self)
	end

	function pMeta:NotifyKill (strID)
		sandbox.KillProgress (strID, self)
	end

	function pMeta:ChatAddText (...)
	...
	local iDelay = 0
	chat.SysMessage = function (...)
		net.Start ("chat.AddText")
			net.WriteTable ({Color (255, 255, 0), "System Announcement", Color (255, 255, 255), ": ", ...})
		net.Broadcast ()
	...
	sound.PlayURL = function (strURL)
		net.Start ("sound.PlayURL")
			net.WriteString (strURL)
		net.Broadcast ()
	end
	sound.PlayFile = function (strPATH)
		net.Start ("sound.PlayFile")
			net.WriteString (strPATH)
		net.Broadcast ()
	end

	Say = function (strMessage)
		RunConsoleCommand ("say", strMessage)
	end
else
	notification.Active = {}

	local NotifySound_Manifest = {
		[NOTIFY_GENERIC] = "weapons/ar2/ar2_empty.wav",
		[NOTIFY_ERROR]   = "buttons/button11.wav",
		[NOTIFY_UNDO]    = "garrysmod/content_downloaded.wav",
		[NOTIFY_HINT]    = "buttons/button1.wav",
		[NOTIFY_CLEANUP] = "buttons/button4.wav",
		[-1]             = "ambient/alarms/klaxon1.wav",
	}

	net.Receive ("player/Console", function ()
		strMessage = net.ReadString ()

		if epoe and epoe.print then
			epoe.print (strMessage)
		else
			print (strMessage)
		end
	end)

	net.Receive ("player/Notify", function ()
		strMessage = net.ReadString ()
		iType      = net.ReadInt    (3)
		iLength    = net.ReadInt    (6)

		surface.PlaySound      (NotifySound_Manifest [iType] and NotifySound_Manifest [iType] or NotifySound_Manifest [-1])
		notification.AddLegacy (strMessage, NotifySound_Manifest [iType] and iType or NOTIFY_HINT, iLength)
	end)

	net.Receive ("player/NotifyProgress", function ()
		strMessage = net.ReadString ()
		strID      = net.ReadString ()
		iLength    = net.ReadInt    (32)


		if iLength ~= 0 then
			timer.Simple (iLength, function ()
				notification.Kill (strID)
			end)
		end

		surface.PlaySound        (NotifySound_Manifest [-1])
		notification.AddProgress (strID, strMessage)
	end)

	net.Receive ("player/NotifyProgressKill", function ()
		strID = net.ReadString ()
		notification.Kill (strID)
	end)
	...
	net.Receive ("sound.PlayURL", function ()
		sound.PlayURL (net.ReadString (), "mono", function () end)
	end)

	net.Receive ("sound.PlayFile", function ()
		sound.PlayFile (net.ReadString (), "mono", function () end)
	end)

	Say = function (strMessage)
		LocalPlayer ():ConCommand (("say %s"):format (strMessage))
	end
end
...
function easylua.FindPlayer(str)
	...
	-- community id
	if #str == 17 and IsValid (player.GetBySteamID64 (str)) then
...
do -- all
	...
	they = CreateAllFunction(function(v) return v:GetClass () == this:GetClass () end)
