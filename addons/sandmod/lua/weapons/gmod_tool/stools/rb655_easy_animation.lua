	...
	timer.Create( tid, delay, 1, function()
		...
		if ( loop == true && IsValid( ent ) ) then
			timer.Adjust( tid, ent:SequenceDuration() / speed, 0, function()
				if ( !IsValid( ent ) ) then timer.Destroy( tid ) return end
				PlayAnimationBase( ent, anim, speed )
			end )
		end
	end )
...
function TOOL:SetSelecetedEntity( ent )
	...
	if ( self.SelecetedEntity == ent ) then return end
...
function TOOL:Reload( trace )
	if ( SERVER ) then
		...
		// Destroy all timers.
		for i=0, UniqueID do timer.Destroy( "rb655_animation_loop_" .. self:GetOwner():UniqueID() .. "-" .. i ) UniqueID = 0 end
		timer.Destroy( "rb655_animation_loop_preview" .. self:GetOwner():UniqueID() )
	end
	return true
end
...
if ( SERVER ) then
	...
	concommand.Add( "rb655_easy_animation_anim_do", function( ply, cmd, args )
		...
		for i = 0, UniqueID do timer.Destroy( "rb655_animation_loop_" .. ply:UniqueID() .. "-" .. i ) UniqueID = 0 end
		timer.Destroy( "rb655_animation_loop_preview" .. ply:UniqueID() )
...
language.Add( "tool.rb655_easy_animation.prop", "Props cannot be animated properly! Open context menu (Hold C) > right click on ragdoll > Make Animatable" )
...
function TOOL.BuildCPanel( panel, ent )
	...
	elseif ( IsEntValid( ent ) ) then
		...
		if ( GetConVarNumber( "ai_disabled" ) == 0 and ent:IsNPC() ) then panel:AddControl( "Label", {Text = "#tool.rb655_easy_animation.ai"} ) fine = false end
		if ( ent:GetClass() == "prop_ragdoll" ) then panel:AddControl( "Label", {Text = "#tool.rb655_easy_animation.ragdoll"} ) fine = false end
		if ( ent:GetClass() == "prop_physics" || ent:GetClass() == "prop_physics_multiplayer" || ent:GetClass() == "prop_physics_override" ) then panel:AddControl( "Label", {Text = "#tool.rb655_easy_animation.prop"} ) end
		...
		if ( fine ) then panel:AddControl( "ListBox", { Label = "#tool.rb655_easy_animation.animations", Options = t, Height = 225 } ) end
	elseif ( !IsEntValid( ent ) ) then
	...
	panel:AddControl( "Checkbox", { Label = "#tool.rb655_easy_animation.noglow", Command = "rb655_easy_animation_noglow" } )
end
...
function PANEL:Paint( w, h )
	...
	local w = surface.GetTextSize( "#" .. self.ent:GetClass() )
	draw.SimpleText( " #" .. self.ent:EntIndex(), "DermaDefault", 24 + w, 0, Color( 255, 255, 255, 255 ) )

	local w = surface.GetTextSize( "#rb655_anim_" .. self.anim )
	local t = " [ S: " .. self.speed .. ", D: " .. self.delay
	if ( self.loop ) then t = t .. ", Looping" end
	draw.SimpleText( t .. " ]", "DermaDefault", 24 + w, 10, Color( 255, 255, 255, 255 ) )
end
